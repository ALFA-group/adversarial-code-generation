...
class SubtokenStatistic:
    ...
    def update(self, other_statistic: "SubtokenStatistic"):
        ...
        self.all_toks += other_statistic.all_toks
        self.wrong_toks += other_statistic.wrong_toks

    def calculate_metrics(self, group: str = None) -> Dict[str, int]:
        precision, recall, f1, asr = 0, 0, 0, 0
        ...
        if self.all_toks > 0:
            asr = self.wrong_toks/self.all_toks
        metrics_dict = {"precision": precision, "recall": recall, "f1": f1, "asr":asr}
    ...
    def calculate_statistic(
        true_subtokens: torch.Tensor, predicted_subtokens: torch.Tensor, skip: List[int] = None, label_to_id: Dict[int, str] = None
    ) -> "SubtokenStatistic":
        ...
        id_to_label = {label_to_id[i]:i for i in label_to_id}
        if skip is None:
            skip = []
        ...
        for batch_idx in range(batch_size):
            ...
            # ASR computation assumes that all inputs are exact matches
            subtoken_statistic.all_toks += len(gt_seq)
            for i in range(min(len(pred_seq), len(gt_seq))):
                if pred_seq[i] != gt_seq[i]:
                    subtoken_statistic.wrong_toks += 1

            if len(gt_seq) == len(pred_seq) and all([g == p for g, p in zip(gt_seq, pred_seq)]):
                subtoken_statistic.true_positive += len(gt_seq)
                continue
            # print(gt_seq, pred_seq)
