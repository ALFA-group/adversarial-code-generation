...
class BufferedPathContext:
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, numpy.ndarray], numpy.ndarray, int]:
        ...
        return item_contexts, self.labels[:, [idx]], self.contexts_per_label[idx], self.filenames[idx]

    def dump(self, path: str):
        with open(path, "wb") as pickle_file:
            pickle.dump((self.contexts, self.labels, self.contexts_per_label, self.filenames), pickle_file)
    ...
    def create_from_lists(
        ...
        return BufferedPathContext(contexts, labels, contexts_per_label, filenames)
