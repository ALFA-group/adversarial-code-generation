	...
	def __next__(self) -> Tuple[Dict[str, numpy.ndarray], numpy.ndarray, int]:
		...
		if self._cur_sample_idx == len(self._order):
			...
			if self._cur_file_idx >= self._end_file_idx:
				raise StopIteration()
			self._prepare_buffer(self._cur_file_idx)
		context, label, paths_for_label, filename = self._cur_buffered_path_context[self._order[self._cur_sample_idx]]
		...
		return context, label, paths_for_label, filename
...
class PathContextBatch:
	def __init__(self, samples: List[Tuple[Dict[str, numpy.ndarray], numpy.ndarray, int, str]]):
		self.context = {
		...
		self.filenames = [sample[3] for sample in samples]
