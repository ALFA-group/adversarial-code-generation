...
def train(
	dataset_name: str, model_name: str, expt_dir: str, data_folder: str, num_workers: int = 0, is_test: bool = False, resume_from_checkpoint: str = None
):
	...
	# define logger
	wandb_logger = WandbLogger(project=f"{model_name}-{dataset_name}", log_model=True, offline=True)
...
if __name__ == "__main__":
	...
	arg_parser.add_argument("--expt_dir", type=str)
	arg_parser.add_argument("--data_folder", type=str)
	args = arg_parser.parse_args()

	train(args.data, args.model, args.expt_dir, args.data_folder, args.n_workers, args.test, args.resume)
