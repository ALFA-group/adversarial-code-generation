...
class Code2Seq(BaseCodeModel):
    ...
    def forward(
        ...
        return self.decoder(self.encoder(samples, use_embedding_layer=use_embedding_layer, already_one_hot=already_one_hot), paths_for_label, output_length, target_sequence)
    
    def copy_weights(self):
        self.encoder.copy_weights()
    ...
    def _calculate_metric(self, logits: torch.Tensor, labels: torch.Tensor) -> SubtokenStatistic:
        with torch.no_grad():
            ...
            statistic = SubtokenStatistic().calculate_statistic(
                labels, prediction, [self.vocab.label_to_id[t] for t in [PAD, SOS, EOS, UNK]], self.vocab.label_to_id
            )
        return statistic
